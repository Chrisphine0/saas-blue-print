...
export function FavoritesList({ favorites, buyerId }: FavoritesListProps) {
  ...
  const addToCart = async (productId: string, minQty: number) => {
    ...
    try {
      ...
      // ⚡ DISPATCH EVENT TO HEADER FOR INSTANT UPDATE
  ...
  if (favorites.length === 0) {
    return (
      <div className="text-center py-12 bg-white rounded-lg border border-dashed">
        ...
        <Button onClick={() => router.push("/buyer/dashboard/catalog")}>
          Browse Products
        </Button>
      </div>
    )
  ...
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {favorites.map((favorite) => {
        ...
        // In Favorites, inventory is often the first item in an array depending on your Join
        const inventory = Array.isArray(product.inventory) ? product.inventory[0] : product.inventory

        // ✅ REPLICATED STOCK LOGIC
        ...
        return (
          <Card key={favorite.id} className="overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              {/* ✅ Accurate Available Display */}
              <p className={`text-xs mt-1 ${isOutOfStock ? 'text-red-500 font-medium' : 'text-gray-500'}`}>
                Available: {availableForSale} {product.unit_of_measure}
              </p>
            </CardContent>
